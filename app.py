...
def extract_keywords(text):
    ...
    for word in words:
        # ❌ Skip common useless words
        if word in COMMON_WORDS:
            continue

        # ✅ Keep only suspicious hints
        if word in SUSPICIOUS_HINTS:
...
IMPERSONATION_TARGETS = {
    ...
    "incometax": "Income Tax Dept",
    "parivahan": "Transport Ministry",
...
INDIAN_DOC_SCAMS = [
    ...
    "needs biometrics update",
    "needs document update",
    "avoid deactivation",
    "update to continue",
    "pan card inoperative",
    "pan inoperative",
    ...
    "gstin blocked",
    "passport dispatch halted",
    "police verification failed",
    "passport file on hold",
...
OTP_PHISHING_SCAMS = [
    ...
    "unusual login",
    "share this code",
    "verification code for amazon",
    "verification code for flipkart",
    "amount deducted",
    "request to pay",
    "approve request",
]
...
def inspect_link(url):
    ...
    try:
        ...
        if response.history:
            ...
            if ".apk" in final_url:
                risks.append(
                    "⛔ **Malware Alert:** Redirected to an App download (.apk)"
                )
                score_add += 5
        if response.status_code != 200:
            risks.append(
                f"⚠️ **Suspicious:** Site returned Error {response.status_code} (Likely taken down)."
            )
            score_add += 2
...
def get_threat_analysis(text, source_type):
    ...
    safety_bonus = safe_word_count // 3
    if safety_bonus > 0:
        score -= safety_bonus
        flags.append(
            f"✅ **Natural Language:** Found {safe_word_count} common words, decreasing suspiciousness."
        )

    # ... The rest of your code (Whitelist, Links, etc.) follows here
    # --- A. WHITELIST CHECK ---
    for safe_domain in OFFICIAL_DOMAINS:
        if safe_domain in text_lower:
            is_safe_source = True
            flags.append(
                f"✅ **Verified Source:** Message contains official domain '{safe_domain}'."
            )
            score -= 100
            break
    ...
    if detected_impersonation and not is_safe_source:
        flags.append(
            f"🏢 **Entity Detection:** Message mentions {', '.join(detected_impersonation)}."
        )
        flags.append(
            f"⚠️ **Warning:** Verify this is truly from their official app/number."
        )
    ...
    for category, patterns in SAFE_PATTERNS.items():
        for phrase in patterns:
            if phrase in text_lower:
                score -= 3
                flags.append(
                    f"✅ **Likely Safe:** Context appears to be legitimate ({category})."
                )
                break
    ...
    for phrase in SENSITIVE_WARNINGS:
        if phrase in text_lower:
            score += 1
            flags.append(
                f"🔒 **Security Alert:** Message contains SENSITIVE CODE ('{phrase}'). **DO NOT SHARE** this with anyone."
            )
            break
    ...
    try:
        ...
        cursor.execute("SELECT keywords FROM scam_reports")
        reports = cursor.fetchall()

        scanned_keywords = extract_keywords(text)

        if scanned_keywords:
            for report in reports:
                db_keywords = report[0]

                if db_keywords and any(
                    word in scanned_keywords.split() for word in db_keywords.split()
                ):
                    score += 8
                    flags.append("🚨 COMMUNITY ALERT: Similar scam reported by users.")
                    break
    ...
    threat_lists = [
        (
            DIGITAL_ARREST_SCAMS,
            5,
            "🚨 **DIGITAL ARREST SCAM:** Fake Police/Customs threat detected",
            "Police/Legal",
        ),
        (
            POLICE_CHALLAN_SCAMS,
            5,
            "⚠️ GOV IMPOSTER: Fake Challan/Legal notice",
            "Traffic Challan",
        ),
        (
            VIRUS_TROJAN_SCAMS,
            5,
            "⛔ **MALWARE ALERT:** Virus/Trojan/Worm threat detected",
            "Tech Support",
        ),
        (
            IDENTITY_THEFT_SCAMS,
            5,
            "🚨 **IDENTITY THEFT:** Impersonation/Signature fraud detected",
            "Identity",
        ),
        (
            HACKING_TERRORISM_SCAMS,
            5,
            "🚨 **CYBER TERRORISM/HACKING:** Critical infrastructure/Unity threat detected",
            "Terrorism",
        ),
        (
            SOCIAL_MEDIA_CRIMES,
            3,
            "⚠️ SOCIAL CRIME: Harassment/Impersonation/Matrimonial fraud",
            "Social Media",
        ),
        (
            CRYPTO_MINING_SCAMS,
            3,
            "⚠️ CRYPTOJACKING: Unauthorized mining threat",
            "Crypto",
        ),
        (
            FAKE_APP_SCAMS,
            4,
            "❌ FAKE APP SCAM: Malicious software promotion detected",
            "Fake App",
        ),
        (
            INVESTMENT_SCAMS,
            3,
            "⚠️ INVESTMENT FRAUD: Suspicious high-return scheme",
            "Investment",
        ),
        (
            SEXTORTION_SCAMS,
            5,
            "⛔ **SEXTORTION ALERT:** Blackmail/Obscenity threat detected",
            "Extortion",
        ),
        ...
        (
            INDIAN_DOC_SCAMS,
            4,
            "❌ GOV DOC SCAM: Official Identity Fraud detected",
            "Govt Document",
        ),
        (
            OTP_PHISHING_SCAMS,
            3,
            "⚠️ OTP/PANIC SCAM: Fake transaction or panic trigger",
            "Banking/OTP",
        ),
        (
            INDIAN_BANKING_SCAMS,
            3,
            "❌ BANK SCAM (India): Panic tactic detected",
            "Banking",
        ),
        (
            INDIAN_UTILITY_SCAMS,
            4,
            "❌ UTILITY SCAM: Fake disconnection threat",
            "Utility Bill",
        ),
        (
            INDIAN_LOAN_APP_SCAMS,
            5,
            "🚨 **LOAN APP SCAM:** Harassment/Blackmail detected",
            "Loan App",
        ),
        ...
        (
            UPI_LOTTERY_SCAMS,
            3,
            "⚠️ UPI/SCHEME FRAUD: Fake Reward/Lottery claim",
            "UPI/Lottery",
        ),
        ...
        (
            GLOBAL_DELIVERY_SCAMS,
            2,
            "📦 DELIVERY SCAM: Fake shipping notification",
            "Delivery",
        ),
        (
            GLOBAL_FINANCE_SCAMS,
            3,
            "💳 PHISHING: Tax/Account Suspicious Activity",
            "Financial",
        ),
        (
            GLOBAL_RELATIONSHIP_SCAMS,
            3,
            "💔 RELATIONSHIP SCAM: Trust/Money trick detected",
            "Romance",
        ),
        (
            GLOBAL_TECH_SUPPORT,
        ...
        (
            SOCIAL_SCAMS,
            3,
            "⚠️ SOCIAL ENGINEERING: Emergency/Money trick detected",
            "Social",
        ),
        (
            PHISHING_KEYWORDS,
            3,
            "⚠️ **PHISHING PATTERN:** Suspicious inheritance/lottery content",
            "Phishing",
        ),
        (
            MALWARE_KEYWORDS,
            5,
            "💾 **MALWARE THREAT:** Dangerous file request detected",
            "Malware",
        ),
    ]
    ...
    if "PDF_PASSWORD_LOCKED" in text:
        ...
        flags.append(
            "🔐 **SECURITY ALERT:** The uploaded PDF is password protected or corrupted."
        )
        ...
        for url in urls:
            ...
            for ext in MALWARE_EXTENSIONS:
                if url_lower.endswith(ext):
                    score += 10
                    flags.append(
                        f"⛔ **MALWARE ALERT:** Link ends in dangerous extension '{ext}'"
                    )
    ...
    if urls and not is_safe_source:
        ...
        if detected_threat_type:
            flags.append(
                f"⛔ CRITICAL: This message appears to be about **{detected_threat_type}**, but the link provided does NOT match our official records for that category."
            )
        else:
            flags.append(
                "⛔ CRITICAL POLICY: The link provided is NOT an official government or banking domain. We treat all unverified links as high risk."
            )
...
# --- REPORT SCAM ROUTE (AUTOMATIC SAVING) ---
...
def report_scam():
    try:
        ...
        # 1. VALIDATION: Check for minimum 20 characters
        if len(content) < 20:
            return jsonify(
                {
                    "status": "error",
                    "message": f"Report too short ({len(content)}/20). Please provide at least 20 characters.",
                }
            )
        ...
        # 2. KEYWORD EXTRACTION: Get keywords from the text
        # (Assuming extract_keywords returns a tuple: keywords, count)
        keywords = extract_keywords(content)

        # 3. DATABASE: Connect and save the report
        conn = sqlite3.connect("scam_reports.db")
        cursor = conn.cursor()

        # Matches the table schema: content, keywords, source, timestamp
        cursor.execute(
            """
            INSERT INTO scam_reports (content, keywords, source, timestamp)
            VALUES (?, ?, ?, ?)
        """,
            (content, keywords, source, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
...
def get_stats():
    ...
    try:
        ...
        cursor.execute("SELECT timestamp FROM scam_reports ORDER BY id DESC LIMIT 1")
        last = cursor.fetchone()

        if last:
            last_reported = last[0]
